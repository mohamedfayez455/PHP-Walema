...
class EnquiryFormBuilderInput extends Model {

	protected $fillable = ['input_id', 'published_on_profile', 'publish'];
	...
	public function InputAttributes() {
		return $this->hasMany('App\EnquiryFormBuilderInputAttribute', 'input_id', 'id');
	}

	public function childrens() {
		return $this->hasMany('App\EnquiryFormBuilderInputChildrens', 'enquiry_input_id', 'id');
	}
	...
	public static function validate_option() {
		...
		foreach (request()->all() as $attr => $val) {
			...
			if (preg_match('/^item[0-9]$/i', $attr) and !array_key_exists($name, ['categories_id', 'types_id'])) {
				request()->validate([
					$attr => 'required|string|max:30',
				]);
	...
	public static function validate_attribute($flag = null) {
		...
		$enquiry_input = EnquiryFormBuilderInput::find(request('input_id'));

		if ($enquiry_input) {

			$field = InputFields::find($enquiry_input->input_id);
		...
		if ($field) {
			...
			if ($flag == 'update') {
				...
				request()->validate([
					...
					'name' => 'required|alpha_dash|max:25|unique:enquiry_form_builder_input_attributes,value',
				]);
			}
	...
	public static function createInput($request, $input_id) {
		...
		$enquiryInput = EnquiryFormBuilderInput::create([
			'input_id' => $input_id,
			'publish' => $publish,
		]);

		$request = $request->except(['input_id', '_token', 'publish', '_method']);
		...
		foreach ($request as $key => $value) {
			...
			if ($value) {
				...
				if (!$attribute_id) {
					...
					if (preg_match('/^item[0-9]$/i', $key) || preg_match('/^option[0-9]$/i', $key)) {
						...
						if ($input_id) {
							...
							EnquiryFormBuilderInputChildrens::create([
								...
								'enquiry_input_id' => $enquiryInput->id,
								'input_id' => $input_id,
								'name' => ucfirst($value),
								'value' => $value,
							]);
					...
					if ($enquiryInput->input->type == 'checkbox' and $key == 'name') {
		...
		if ($anotherInputAttr != []) {
			...
			self::addAnotherField($anotherInputAttr, $input_id, $publish);

		}

	}

	public static function updateInput($request, $enquiry_input_id) {

		$enquiryInput = EnquiryFormBuilderInput::find($enquiry_input_id);
		...
		if ($enquiryInput) {
			...
			$request = $request->except(['input_id', '_token', 'published_on_profile', 'publish', '_method']);
			...
			if ($anotherInputAttr != []) {
				...
				self::addAnotherField($anotherInputAttr, $enquiryInput->input_id, $publish);
	...
	public static function addAnotherField($anotherInputAttr, $input_id, $publish) {
		$enquiryInput = EnquiryFormBuilderInput::create([
			'input_id' => $input_id,
			'publish' => $publish,
		]);
	...
	public static function inputsOfEnquiryWithAttribute($page = null) {
		...
		if ($page == null) {
			$inputs = EnquiryFormBuilderInput::all();
		} else {
			$inputs = EnquiryFormBuilderInput::where($key, 1)->get();
		}
		...
		if ($page == 'edit_profile') {
			...
			$user = enquiry()->user->toArray();
			$enquiry = enquiry()->toArray();

			$userAttribute = array_keys($user);
			$conAttribute = array_keys(enquiry()->toArray());
			...
			foreach ($conAttribute as $attribute) {
				...
				$values[$attribute] = $enquiry[$attribute];

			}

			$additional_data = enquiry()->toArray()['additional_data'] ?: [];
	...
	public static function findInputAttributs($input_id) {
		...
		$enquiryInput = EnquiryFormBuilderInput::where('id', $input_id)->first();

		$attributes = InputFields::findInputAttributs($enquiryInput->input_id);

		$enquiryInputAttributes = $enquiryInput->InputAttributes;
		...
		foreach ($attributes as $attribute) {
			...
			foreach ($enquiryInputAttributes as $enquiryInputAttribute) {

				if ($attribute['attribute'] == $enquiryInputAttribute->attribute->attribute) {

					$attribute['value'] = explode('[]', $enquiryInputAttribute->value)[0];

				}

			}

			$input_id = $enquiryInput->input_id;

			$enquiryinputs = EnquiryFormBuilderInput::where('input_id', $input_id)->get()->toArray();
			$number = count($enquiryinputs);

			if ($number == 2 and $attribute['attribute'] == 'is_confirm' and $attribute['value'] == 'on') {
				continue;
			}

			$enquiryInputExistsMoreOne = false;

			foreach ($enquiryinputs as $input) {

				if ($input['id'] == $enquiryInput->id) {

					$enquiryInputExistsMoreOne = true;
					break;
				}
			}

			if ($number == 2 and $enquiryInputExistsMoreOne and $attribute['attribute'] == 'is_confirm') {
				continue;
			}
		...
		return [$attributesArray, $enquiryInput->publish];
	}
	...
	public static function findInputChildrens($input_id) {
		...
		$childrens = InputFields::findInputChildrens($enquiryInput->input_id);

		$enquiryInputChilds = $enquiryInput->childrens->toArray();

		$childsArray = [];

		foreach ($childrens as $child) {

			foreach ($enquiryInputChilds as $enquiry) {

				$enquiry['type'] = $child['type'];
				$enquiry['name'] = $child['name'];
